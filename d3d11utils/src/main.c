...
int main(void)
{
    ...
    // NOTE: Create d3d11 device and context
    {
        UINT flags = 0;
        ...
        hr = D3D11CreateDevice(0, D3D_DRIVER_TYPE_HARDWARE, 0, flags, levels, sizeof(levels) / sizeof(levels[0]), D3D11_SDK_VERSION, &device, 0, &context);
        d3d11u_assert_hr(hr);
    }

    // NOTE: Enable debug break on D3D11 and DXGI API errors
    {
        {
            ID3D11InfoQueue* info = 0;
            hr = ID3D11Device_QueryInterface(device, &IID_ID3D11InfoQueue, &info); // TODO: can fail
            hr = ID3D11InfoQueue_SetBreakOnSeverity(info, D3D11_MESSAGE_SEVERITY_CORRUPTION, 1); // TODO: can fail
            hr = ID3D11InfoQueue_SetBreakOnSeverity(info, D3D11_MESSAGE_SEVERITY_ERROR, 1); // TODO: can fail
            ID3D11InfoQueue_Release(info);
        }

        {
            IDXGIInfoQueue* info = 0;
            hr = DXGIGetDebugInterface1(0, &IID_IDXGIInfoQueue, &info); // TODO: can fail
            hr = IDXGIInfoQueue_SetBreakOnSeverity(info, DXGI_DEBUG_ALL, DXGI_INFO_QUEUE_MESSAGE_SEVERITY_CORRUPTION, 1); // TODO: can fail
            hr = IDXGIInfoQueue_SetBreakOnSeverity(info, DXGI_DEBUG_ALL, DXGI_INFO_QUEUE_MESSAGE_SEVERITY_ERROR, 1); // TODO: can fail
    ...
    IDXGISwapChain1* swap_chain = 0;
    {
        ...
        hr = IDXGIFactory2_CreateSwapChainForHwnd(factory, device, window, &desc, 0, 0, &swap_chain);
        d3d11u_assert_hr(hr);

        // NOTE: disable Alt+Enter changing monitor resolution to match window size
        hr = IDXGIFactory_MakeWindowAssociation(factory, window, DXGI_MWA_NO_ALT_ENTER); // TODO: may fail
    ...
    while (is_running)
    {
        ...
        // NOTE: present
        {
            hr = IDXGISwapChain1_Present(swap_chain, 1, 0); // TODO: can fail
        }
    }
